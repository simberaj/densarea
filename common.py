...
def hasExt(path):
  return path.rfind('.') > max(path.rfind('/'), path.rfind(os.sep))
...
def listLayers(workspace):
  '''Lists data sources in a given workspace.'''
...
def split(listStr):
  '''Splits the ArcPy semicolon-delimited arguments to a list.'''
  parts = listStr.split(';')
  i = 0
  while i < len(parts):
...
def clearFields(layer, exclude=[]):
  '''Deletes all the fields in a given attribute table,
  possibly for the given exceptions. Retains required fields such as OIDs.'''
...
def ensureShapeLengthField(layer, name=SHAPE_LENGTH_FLD):
  ...
  if name.lower() not in [fld.lower() for fld in fieldList(layer)]:
    calcField(layer, name, '!shape.length!', float)
  return name
    
def ensureShapeAreaField(layer):
  if SHAPE_AREA_FLD.lower() not in [fld.lower() for fld in fieldList(layer)]:
...
class PathManager:
  '''A temporary file manager that takes care of temporary
  feature class/layer/field/folder creation and deletion.
  Uses tmp_ prefixes, do not use them yourself.'''
